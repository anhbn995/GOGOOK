...
def rotate_angle(org_matrix, angle):
    ...
    if angle == "90":
        ...
        return m3
def data_gen_not_batch(image_path, mask_path,list_id,augment=True,batch_size = 4, valid=False, val_percentage=0):
    num = len(list_id)
    # new_image_width = 512
    ...
    while True:
        ...
        while len(x)<batch_size:
            ...
            for i in range(1000):
                ...
                with rasterio.open(mask_fn, 'r') as t:
                    mask = t.read().astype(np.uint8)
                t.close()
                if mask.max()!=1 or valid: break
                else:
                    if random.randint(0,100)<30: break
            ...
            with rasterio.open(image_fn, 'r') as f1:
                ...
                image = result/np.finfo(np.float16).max #/255
                with rasterio.open(mask_fn, 'r') as f:
                    mask = f.read().astype(np.uint8)
                    mask_full = np.zeros((INPUT_SIZE, INPUT_SIZE, 3),dtype=np.uint8)
                    mask2 = np.array((mask[0]==2).astype(np.uint8)) #255
                    mask3 = np.array((mask[0]==3).astype(np.uint8)) #255
                    mask1 = ((mask2+mask3)==0).astype(np.uint8)
                    mask_full[...,0] = mask1   # 1st band red = backgroud
                    mask_full[...,1] = mask2   # 2nd band red = cloud
                    mask_full[...,2] = mask3   # 3rd band blue = shadow
                    mask = mask_full
                    if augment:
                    ...
                    x.append(image)
                    y.append(mask)
        yield np.array(x, dtype=np.float16), np.array(y, dtype=np.uint8)
