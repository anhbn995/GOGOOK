            ...
            def process(coordinates):
                ...
                if np.count_nonzero(image_detect) > 0:
                    if len(np.unique(image_detect)) <= 2:
                        ...
                        y_pred = model.predict(image_detect[np.newaxis,...]/np.finfo(np.float16).max)[0]
...
if __name__=="__main__":
    ...
    img_path = r"/home/geoai/eodata/cloud_detect_tensorflow/raw_final/232b3e77828845b49d76ab1a83b765ef.tif"
    ...
    model = build_model((None,None,4), 46)
