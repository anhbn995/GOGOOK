...
#     Augmen = Augmentations()
    ...
    my_model = Model_U2Net(512, 4)
    my_model.summary()
    # print(z)
    # my_model = Model_UNet3plus(480, 3)
    # my_model = Adalsn(480, 3)
    #my_model.load_weights(r'/home/skymap/data/Bahrain_change/set4/shp_Set4/train_other/weights/u2net_other_512_s1234_v2.h5')
    ...
    data_train = glob.glob('/home/skm/public_mount/tmp_ducanh/CAOHOCCCCCC/SplitTrainVal_Tree/train/image/*.tif')
    data_val = glob.glob('/home/skm/public_mount/tmp_ducanh/CAOHOCCCCCC/SplitTrainVal_Tree/val/image/*.tif')
    TRAIN_LOGDIR = '/home/skm/SKM/WORK/ALL_CODE/WorkSpaceDucAnh/WorkSkyHay/u2net/Train_CAOHOC_U2net/Tree/log'
    NAME_MODEL_H5 = "CAOHOC_U2net_Tree.h5"
    
    TRAIN_SAVE_WEIGHT = TRAIN_LOGDIR.replace('log','weights')
    os.makedirs(TRAIN_LOGDIR, exist_ok=True)
    os.makedirs(TRAIN_SAVE_WEIGHT, exist_ok=True)
    ...
    for epoch in range(TRAIN_EPOCHS):
        ...
        with tqdm(total=len_val,desc=f'Val',postfix=dict,mininterval=0.3) as pbar:
            ...
            if best_val_loss>=total_val/len_val:
                my_model.save_weights(os.path.join(TRAIN_SAVE_WEIGHT, NAME_MODEL_H5))
