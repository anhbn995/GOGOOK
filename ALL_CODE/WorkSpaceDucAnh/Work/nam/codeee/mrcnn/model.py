...
def mb_conv_block(inputs, block_args, activation, drop_rate=None, prefix='', freeze_bn=False):
    ...
    x = layers.DepthwiseConv2D(block_args.kernel_size, strides=block_args.strides, padding='same', use_bias=False,
    ...
    if has_se:
        ...
        se_tensor = layers.Conv2D(num_reduced_filters, 1, activation=activation, padding='same', use_bias=True,
...
def EfficientNet(img_input,
    ...
    # Build stem
    x = img_input
    x = layers.Conv2D(round_filters(32, width_coefficient, depth_divisor), 3, strides=(2, 2), padding='same', use_bias=False, kernel_initializer=CONV_KERNEL_INITIALIZER, name='stem_convxxx')(x)
...
def EfficientNetB0(input_tensor):
    return EfficientNet(input_tensor, 1.0, 1.0, 224, 0.2,  model_name='efficientnet-b0')


def EfficientNetB1(input_tensor):
    return EfficientNet(input_tensor, 1.0, 1.1, 240, 0.2, model_name='efficientnet-b1')


def EfficientNetB2(input_tensor):
    return EfficientNet(input_tensor, 1.1, 1.2, 260, 0.3, model_name='efficientnet-b2')


def EfficientNetB3(input_tensor):
    return EfficientNet(input_tensor, 1.2, 1.4, 300, 0.3, model_name='efficientnet-b3')


def EfficientNetB4(input_tensor):
    return EfficientNet(input_tensor, 1.4, 1.8, 380, 0.4, model_name='efficientnet-b4')


def EfficientNetB5(input_tensor):
    return EfficientNet(input_tensor, 1.6, 2.2, 456, 0.4, model_name='efficientnet-b5')


def EfficientNetB6(input_tensor):
    return EfficientNet(input_tensor, 1.8, 2.6, 528, 0.5, model_name='efficientnet-b6')


def EfficientNetB7(input_tensor):
    return EfficientNet(input_tensor, 2.0, 3.1, 600, 0.5, model_name='efficientnet-b7')
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    x = KL.Conv2D(64, (7, 7), strides=(2, 2), name='conv1xxx', use_bias=True)(x)
...
def smooth_l1_loss(y_true, y_pred):
    ...
    loss = (less_than_one * sigma_squared * 0.5 * diff**2) + (1 - less_than_one) * (diff - 0.5 / sigma_squared)
    return loss
...
#             _, C2, C3, C4, C5 = EfficientNetB6(input_image)
        # Top-down Layers
...
#         file_hash = 'a546eb3f09edca6d6fff842cad938720'
#         weights_path = get_file(file_name, IMAGENET_WEIGHTS_PATH + file_name,
#                                             cache_subdir='models',  file_hash=file_hash)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, augment = None,
        ...
        if os.name is 'nt':
            ...
            workers = multiprocessing.cpu_count()*3//4
