...
def get_range_value_planet_vung_miner(img, range_band=[2500,2500,2500,6000]): #v1: [2500,2500,2500,6000]  v2: [4000,4000,4000,6500]
    data = np.empty(img.shape)
    for i in range(4):
        data[i] = img[i]/range_band[i]
        data[i][data[i] > 1] = 1
        # mask = (data[i] <= 1)
        # data[i] = data[i]*mask
    return data[:4]
...
# def get_data_fake(path_train, size=256, numbands=4, train=True, uint8_type = False):
#     # label_count=len(labels) + 1
#     ids = next(os.walk(path_train + "/images"))[2]
    
    
#     ids = [os.path.basename(fp) for fp in glob.glob(os.path.join(path_train+"/images", "*.tif"))]
...
#                 onehot[..., 0] = (mask==255).astype(np.uint8)
...
def get_data(path_train, size=256, numbands=4, train=True, uint8_type = False):
    ...
    for n, id_ in enumerate(ids):
        # Load images
        x_img = rasterio.open(path_train + '/images/' + id_).read()[:4]
        if not uint8_type:
            # x_img = get_range_value_planet_vung_miner(x_img)
            x_img = get_range_value_planet_vung_miner(x_img)
        x_img = np.transpose(x_img, (1,2,0))
        # Load masks
        if train:
