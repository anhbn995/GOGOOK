...
def predict(model, fp_i, fp_P, md_s, tmd):
                ...
                for x_, y_, x_c, y_c, s_x, s_y in l_coor:
                    ...
                    if np.count_nonzero(img_d) > 0:
                        if len(np.unique(img_d)) <= 2:
                            ...
                            img_, c, v = config.get_value(img_d, tmd)
                            if c:
                                r_p = model.predict(img_)[0]
                            else:
                                r_p = model.predict(img_)
                            r_p = (r_p[0,...,0] > v).astype(np.uint8)
                            y = r_p[m].reshape(size_s)
...
if __name__=="__main__":
    ...
    # fp_model = r'/home/skm/SKM16/X/Lo/Full_Images_LandslideDetection_8bit_perimage/Model_v2_ThemVungKhongTot/all_model_tensorboard/Model_UNet3plus_v2/model_128/Model_UNet3plus_size128_20230515_091729_cau_truc.h5'
    ...
    fp_predict = r'/home/skm/SKM16/X/Lo/Full_Images_LandslideDetection_8bit_perimage/bo_them_nodata/Img_uint8_crop/333.tif'
    type_model = ['U2NET','U3NET','UNET']
    size_model = 128
    
    model = tf.keras.models.load_model(fp_model)
    predict(model, fp_img, fp_predict, size_model, type_model[type_model.index('UNET')])
