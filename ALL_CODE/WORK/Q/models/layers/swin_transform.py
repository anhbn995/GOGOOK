...
class SwinTransformerBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        if self.shift_size > 0:
            ...
            # To do: pad feature maps to multiples of window size
    ...
    def call(self, x):
        ...
        x = tf.pad(x, paddings=[[0,0], [pad_t,pad_b], [pad_l,pad_r], [0,0]], mode="CONSTANT")
        _, Hp, Wp, _ = x.shape
...
class PatchMerging(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        if pad_input:
            x = tf.pad(x, paddings=([0, 0], [0, H % 2], [0, W % 2], [0, 0]))
...
class PatchEmbed(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        if W % self.patch_size[1] != 0:
            x = tf.pad(x, paddings=[[0, 0], [0, 0], [0, 0], [0, self.patch_size[1] - W % self.patch_size[1]]])
        if H % self.patch_size[0] != 0:
            x = tf.pad(x, paddings=[[0, 0], [0, 0], [0, self.patch_size[0] - H % self.patch_size[0]], [0, 0]])
...
class SwinTransformerModel(tf.keras.Model):
    ...
    def forward_features(self, x):
        ...
        outputs = []
        dim = [self.embed_dim,self.embed_dim*2,self.embed_dim*4,self.embed_dim*8]
        # dim = [192,384,768,1536]
        # dim = [128,256,512,1024]
        for i, layer in enumerate(self.basic_layers):
            x, x1 = layer(x)
            x1 = tf.reshape(x1,shape=(-1, int(math.sqrt(x1.shape[1])), 
                                      int(math.sqrt(x1.shape[1])), dim[i]))
            outputs.append(x1)
        # x = self.norm(x)
        # x = self.avgpool(x)
        # outputs.append(x)
...
if __name__ == "__main__":
    ...
    model = tf.keras.Model(inputs=inputs, outputs=out_backbones)
    model.summary()