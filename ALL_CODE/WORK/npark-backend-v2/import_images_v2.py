...
from ultils import reproject_image, get_pixel_size
...
def statistic_images(image_path, image_forest_path, x_size, y_size, band_red=3):
    range_i = [
        [0., 0.2, 0.4, 0.6],
        [0., 0.2, 0.5, 0.8, 1.1],
        [0., 0.2, 0.4, 0.6, 0.8]
    ...
    # with rasterio.open(image_forest_path) as src_:
    #     savi = src_.read()[0].astype(np.float32)
    # print(savi,'nguyen duc'*100)
    if type(band_red) == type(3):
        band_red_ = band_red
        band_nir = 4
        print("x"*100)
    else:
        band_red_ = band_red[0]
        band_nir = band_red[1]
        print("*"*100)

    with rasterio.open(image_path) as dst:
        red = dst.read(band_red_).astype(np.float32)
        nir = dst.read(band_nir).astype(np.float32)
...
def store_mosaic_image(fid_mosaic_view, mosaic_statistic_image, mosaic_view_image, classification_image, year, month,
    ...
    for type_image in type_images:
        ...
        data = {
            ...
            'name': f'{year}_{month}_{type_image["name"]}',
            'path': abs_path,
            'month': month,
            'year': year,
            'src': src,
        ...
        if 'properties' in type_image:
            data['properties'] = type_image['properties']
        list_images.append(data)

    return list_images


def calc_vegetable_index(image_path, src, temp_folder, initial_aoi, month, year, classification_image,
                         forest_image_path, band_red=3):
    # print('in'*20)
    aoi_id = initial_aoi["id"]
    list_images = []
    ...
    with rasterio.open(image_path) as dst:
        red_arr = dst.read(band_red_).astype(np.float32)
        nir_arr = dst.read(band_nir).astype(np.float32)
    ...
    # with rasterio.open(forest_image_path) as src_tmp:
    #     density = src_tmp.read()[0].astype(np.float32)
    ...
    forest_duc_anh = f'{ROOT_DATA_FOLDER}/data/{year}/{month}/images/{uuid.uuid4().hex}.tif'
    _translate(forest_image_path, forest_duc_anh)
    meta_forest = calc_forest(forest_duc_anh, forest_image, x_size, y_size, classification_image)
    ...
    list_images.append({
        'file_id': fid_density,
        'path': density_image,
        'name': f'{year}_{month}_plant_density',
        'month': month,
        'year': year,
        'properties': {
            "mode": "pseudocolor", "bands": [0, 0, 0], "contrastEnhancement": "stretch_to_minmax",
    ...
    list_images.append({
        'file_id': fid_forest,
        'path': forest_image,
        'name': f'{year}_{month}_forest_cover',
        'month': month,
        'year': year,
        'properties': {
            "mode": "pseudocolor", "bands": [0, 0, 0], "contrastEnhancement": "stretch_to_minmax",
...
def calc_area(fid_mosaic, image_path, image_forest_path, x_size, y_size, band_red=3):
    print('*' * 100)
    properties = statistic_images(image_path, image_forest_path, x_size, y_size, band_red)
    print(properties)
    payload = {
...
if __name__ == '__main__':
    ...
    #             print(x_size, y_size)
    #
    #             calc_area(fid_mosaic, reclass_image_path, image_forest_path, x_size, y_size)
