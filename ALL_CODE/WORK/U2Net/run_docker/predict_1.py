...
model_path2 = r"/home/skm/SKM/WORK/ALL_CODE/WORK/U2Net/log/weights/unet_bce_solarPanel.h5"
model2 = build_model((256,256,4), 32)

model2.load_weights(model_path2)

# v1= /home/skm/SKM_OLD/public/DA/AAA/DuongBoVenBien/
import glob, os
list_fp = glob.glob("/home/skm/SKM16/Work/SonalPanel_ThaiLand/1Ver2_lable2/izmages_8bit_perizmage/images_per95/*.tif")
...
for path_image in tqdm(list_fp):
    if path_image.find('_mask.tif') != -1:
        ...
        out_folder = r"/home/skm/SKM16/Work/SonalPanel_ThaiLand/1Ver2_lable2/izmages_8bit_perizmage/images_per95/build_model"
            ...
            with tqdm(total=len(list_coordinates)) as pbar:
                with rasterio.open(path_predict,'w+', **meta, compress='lzw') as r:
                    for x_off, y_off, x_count, y_count, start_x, start_y in list_coordinates:
                        image_detect = raster.read(window=Window(x_off, y_off, x_count, y_count))[0:4].transpose(1,2,0)
                        ...
                        if np.count_nonzero(image_detect) > 0:
                            if len(np.unique(image_detect)) <= 2:
                                ...
                                print(shape, "okaaaaaaaa", y_pred.shape)
