...
size_crop = int(sys.argv[2])
true_size = int(sys.argv[3])
...
def crop_image(image_id,path_image_crop):
    print(1)
    ...
    # mask_path = os.path.join(foder_image_mask,image_id+'_mask.tif')
...
def main():
    ...
    result = p_cnt.map(partial(crop_image,path_image_crop=path_image_crop), list_id)
    p_cnt.close()
    p_cnt.join()
    return True
...
# 3 tham so dau vao:folder _cut_img, size cut, size truot/ thuong size la 256